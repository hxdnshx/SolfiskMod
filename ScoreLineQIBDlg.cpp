...
static LRESULT ScoreLineView_OnDoubleClick(HWND hParent, HWND hwnd)
{
	...
	if (index >= 0 && index < TH135AddrGetCharCount()) {
...
static void ScoreLineQIBDialog_Reflesh(HWND hDlg)
{
	...
	int sumWinAll, sumLoseAll;
	{
		...
		ScoreLineQIBDialog_RefleshUnit(listWnd, TH135AddrGetCharCount() + 1, sumWinAll, sumLoseAll);
	}
	...
	for (int h = 0; h < _countof(limits); ++h) {
		...
		ScoreLineQIBDialog_RefleshUnit(listWnd, TH135AddrGetCharCount() + 2 + h, sumWinAll, sumLoseAll);
	}
...
static BOOL ScoreLineQIBDialog_InitListView(HWND hDlg)
{
	...
	item.lParam = 0;
	for (int i = 0; i < TH135AddrGetCharCount(); ++i) {
		item.pszText = const_cast<LPTSTR>(TH135AddrGetCharName(i)->abbr);
		item.iItem = i;
	...
	LVPARAMFIELD(item.lParam).charId = TH135AddrGetCharCount();
