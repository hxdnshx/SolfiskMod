...
static const struct {
	...
	//{ LVCFMT_RIGHT,	1,		_T("DebugMsg") },
};

//因為我的無能最後還是把libcmt放進來了,使用的是swscanf- -
...
static void TrackRecordDialog_QueryCallback(SCORELINE_ITEM *item, void *user)
{
	...
	lvitem.pszText = text;
	::wsprintf(text, _T("%d/%02d/%02d %02d:%02d%:%02d:%04d"),
	...
	lvitem.iSubItem = 2;
	lvitem.pszText = const_cast<LPTSTR>(TH135AddrGetCharName(item->p1id)->abbr);
	ListView_SetItem(listWnd, &lvitem);
	...
	lvitem.iSubItem = 5;
	lvitem.pszText = const_cast<LPTSTR>(TH135AddrGetCharName(item->p2id)->abbr);
	ListView_SetItem(listWnd, &lvitem);
...
static LRESULT TrackRecordView_OnDoubleClick(HWND hwndParent, HWND hwnd)
{
	int index = ListView_GetSelectionMark(hwnd);
	if (index >= 0) {
		TCHAR timestamp[MAX_PATH + 1];
		LVITEM item;
		item.mask = LVIF_TEXT | LVIF_PARAM;
		item.iItem = index;
		item.iSubItem = 0;
		item.cchTextMax = _countof(timestamp);
		item.pszText = timestamp;
		...
		::swscanf_s(timestamp,_T("%d/%d/%d %d:%d%:%d:%hd"),
		&systime.wYear, &systime.wMonth, &systime.wDay,
		&systime.wHour, &systime.wMinute, &systime.wSecond,
		&systime.wMilliseconds);
		
		/*
		static wistringstream ws;
		ws.clear();
		ws.str(timestamp);
		wchar_t ch;
		ws>>systime.wYear>>ch>>systime.wMonth>>ch>>systime.wDay//>>_T(" ")
			>>systime.wHour>>ch>>systime.wMinute>>ch>>systime.wSecond>>ch
			>>systime.wMilliseconds;
			*/
		...
		INT64 stamp;
		//stamp=item.lParam;
		//stamp=StrToIntW(timestamp);
		SystemTimeToFileTime(&systime,(LPFILETIME)&stamp);


		COMBOINFO_FILTER_DESC filterDesc;
		filterDesc.battle=stamp;
		filterDesc.fin=1;
		filterDesc.mask=COMBOINFO_FILTER__BATTLE | COMBOINFO_FILTER__FIN;

		ComboDialog_ShowModeless(::GetParent(hwndParent), &filterDesc);
	}
	return FALSE;
}
...
static LRESULT TrackRecordDialog_OnNotify(HWND hDlg, int idCtrl, LPNMHDR pNMHdr)
{
	...
	case IDC_LIST_TRACKRECORD:
		...
		case NM_DBLCLK:
			return TrackRecordView_OnDoubleClick(hDlg, pNMHdr->hwndFrom);
		}
		break;
	}
	return FALSE;
